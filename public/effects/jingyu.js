var RENDERER = {
    ...
    render : function(){
        ...
        // 水体颜色
        this.context.fillStyle = 'rgba(247, 156, 147, 1)';
        
        for(var i = 0, count = this.fishes.length; i < count; i++){
            this.fishes[i].render(this.context);
        }
        this.context.save();
        //鲸鱼进入水体后的显示效果(xor、source-over、multiply 或 screen)
...
FISH.prototype = {
    ...
    render : function(context){
        ...
        // 设置鲸鱼颜色
        context.fillStyle = 'rgba(243, 164, 157, 1)';
        ...
        // 设置鲸鱼鱼鳍颜色
        context.fillStyle = 'rgba(243, 164, 157, 0.6)';
        context.beginPath();
...
// 添加启动和停止函数，供特效系统调用
let whaleContainer = null;

function start() {
    // 使用已有的容器元素，不创建新容器
    whaleContainer = document.getElementById('whale-effect-container');
    
    if (!whaleContainer) {
        console.error('鲸鱼特效容器未找到');
        return;
    }
    
    // 设置容器样式
    whaleContainer.style.position = 'relative';
    whaleContainer.style.width = '100%';
    whaleContainer.style.height = '200px';
    whaleContainer.style.overflow = 'hidden';
    whaleContainer.style.pointerEvents = 'none';
    whaleContainer.style.background = 'transparent';
    
    // 创建特效容器
    const effectContainer = document.createElement('div');
    effectContainer.id = 'jsi-flying-fish-container';
    ...
    whaleContainer.appendChild(effectContainer);
...
function stop() {
    // 移除特效容器
    if (whaleContainer) {
        const effectContainer = whaleContainer.querySelector('#jsi-flying-fish-container');
        if (effectContainer) {
...
// 将函数绑定到window对象
window.startJingyu = start;
window.stopJingyu = stop;